  ...
  <MainLayout>
    <div class="min-h-screen p-4 sm:p-6 lg:p-8">
      ...
      <h1 class="text-2xl sm:text-3xl title text-gray-800 mb-8 text-center">
        ...
        <span class="text-blue-700">{{ formatNameFromEmail(user.email) }}</span>!
      </h1>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10 max-w-4xl mx-auto">
        ...
        >
          <div>
            <h2 class="text-xl font-semibold text-gray-700 mb-2 count-label">Active Job Project</h2>
            <p class="text-5xl font-bold text-blue-600">{{ projects?.length ?? 0 }}</p>
          </div>
        ...
        >
          <div>
            <h2 class="text-xl font-semibold text-gray-700 mb-2 count-label">New Applicants This Week</h2>
            <p class="text-5xl font-bold text-green-600">{{ totalWeeklyApplications ?? 0 }}</p>
          </div>
          <div class="flex-shrink-0">
            <svg
            ...
            >
              <div>
                ...
                <p class="text-sm text-gray-600">
                  Status: <span class="font-semibold text-green-700">Active</span>
                </p>
                <p class="text-xs text-gray-500 mt-1">Posted: {{ formatDate(project.created_at) }}</p>
              </div>
              <div class="mt-2 sm:mt-0 flex space-x-2">
                ...
                >
                  View Applicants ({{ project.applications.length }})
                </a>
              </div>
            </div>
          </div>
                ...
                <div class="flex items-start gap-4 flex-1 min-w-0 mb-4 sm:mb-0">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <p class="text-sm text-gray-700 mt-1">
                      Applied for:
                      <span class="font-medium text-gray-900">{{ application.project.title }}</span>
                    </p>
                    <p class="text-sm text-gray-500">Applied: {{ formatDate(application.created_at) }}</p>
                  </div>
                </div>
