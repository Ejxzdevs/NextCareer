...
export const fetchApiData = async () => {
    try {
        ...
        console.error('NotificationService Error: Failed to fetch notifications (fetchApiData):', err);
        let errorMessage = 'Failed to load notifications due to a network error.';

        if (err.response) {
            console.error('Response Data:', err.response.data);
            console.error('Response Status:', err.response.status);
            errorMessage = `Failed to load notifications: ${err.response.status} - ${err.response.data?.message || 'Server error'}`;
        } else if (err.request) {
...
export const markAllAsRead = async () => {
    try {
        ...
        if (error.response) {
            errorMessage = `Failed to mark all as read: ${error.response.status} - ${error.response.data?.message || 'Server error'}`;
