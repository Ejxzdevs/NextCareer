    ...
    <nav class="bg-[#334EAC] p-4">
      ...
      <div class="flex justify-between items-center font-semibold">
        ...
        <ul class="hidden md:flex space-x-6 text-white items-center">
            ...
            <li><Link
                class="bg-white text-black px-4 py-2 rounded border border-black hover:bg-gray-200 transition duration-200"
                :href="route('register')"
            >
            Register
            </Link>
      ...
      <!-- Mobile Dropdown -->
      <ul v-if="isMenuOpen" class="md:hidden text-white bg-[#7789C7] flex flex-col p-4 space-y-2">
        <Link class="text-gray-300 pointer" :href="route('login')">Login</Link>
        <Link class="text-gray-300 pointer" :href="route('register')">Register</Link>
      </ul>
    </nav>
  </div>
</template>
